...
- (void)setUp
{
    self.actualContentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 60)];
    self.actualContentView.backgroundColor = [UIColor grayColor];
    [self.contentView addSubview:self.actualContentView];
    self.btn_share = [[UIButton alloc] initWithFrame:CGRectMake(250, 15, 50, 30)];
    [self.btn_share setTitle:@"分享" forState:UIControlStateNormal];
    self.btn_share.backgroundColor = [UIColor orangeColor];
    [self.actualContentView addSubview:self.btn_share];
    self.lb_title = [[UILabel alloc] initWithFrame:CGRectMake(20, 10, 200, 40)];
    self.lb_title.textColor = [UIColor whiteColor];
    self.lb_title.backgroundColor = [UIColor clearColor];
    [self.actualContentView addSubview:self.lb_title];
    ...
    self.editable = YES;
    self.moreOptionsButtonTitle = @"重置";
...
- (void)layoutSubviews
{
    ...
    self.moreOptionsButton.frame = CGRectMake(width - menuOptionButtonWidth - CGRectGetWidth(self.deleteButton.frame), 0., menuOptionButtonWidth+15, height);
...
- (CGFloat)menuOptionButtonWidth
{
    ...
    UIFont *fnt = self.moreOptionsButton.titleLabel.font;
    CGFloat width = roundf([string sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:fnt, NSFontAttributeName, nil]].width + 2. * self.menuOptionButtonTitlePadding);//sizeWithFont:self.moreOptionsButton.titleLabel.font
